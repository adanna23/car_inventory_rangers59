...
class User(db.Model, UserMixin):
    ...
    car = db.relationship('Drone', backref = 'owner', lazy = True)
...
class Drone(db.Model):
    ...
    miles = db.Column(db.Numeric(150), nullable = True)
    fuel_capacity = db.Column(db.String(100), nullable = True)
    ...
    def __init__( self, name, description, price, miles, fuel_capacity, max_speed,dimensions, weight, cost_of_prd, series, user_token, id = ''):
        self.id = self.set_id()
        self.name = name
...
class DroneSchema(ma.Schema):
    class Meta:
        fields = ['id','name','description', 'price', 'miles', 'fuel_capacity', 'max_speed', 'dimensions', 'weight', 'cost_of_prd', 'series', 'user_token']
